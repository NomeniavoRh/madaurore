...
class StudentListConseilScreen extends StatefulWidget {
  static const routeName = '/conseil/student-list';

  const StudentListConseilScreen({super.key});

  @override
  State<StudentListConseilScreen> createState() =>
      _StudentListConseilScreenState();
}

class _StudentListConseilScreenState extends State<StudentListConseilScreen> {
  final RequestRepository _requestRepository = RequestRepository();
  String _searchQuery = '';
  String _filterRegion = 'Toutes';
  String _filterMemberStatus = 'Tous';
                ...
                child: SingleChildScrollView(
                  ...
                  child: Row(
                    children: _memberStatuses.map((status) {
                      final isSelected = status == _filterMemberStatus;
                      return Padding(
                        padding: const EdgeInsets.only(right: 8),
                        child: FilterChip(
                          label: Text(status),
                          selected: isSelected,
                          onSelected: (selected) {
                            setState(() => _filterMemberStatus = status);
                          },
                          backgroundColor: Colors.white,
